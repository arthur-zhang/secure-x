...
/// This crate has a runtime dependency on artifacts produced by the `secure-x-ebpf` crate.
...
fn main() {
    ...
    let ebpf_package = packages
        .into_iter()
        .find(|Package { name, .. }| name == "secure-x-ebpf")
        .unwrap();
    ...
    if build_ebpf {
        ...
        // We have a build-dependency on `secure-x-ebpf`, so cargo will automatically rebuild us
        // if `secure-x-ebpf`'s *library* target or any of its dependencies change. Since we
        // depend on `secure-x-ebpf`'s *binary* targets, that only gets us half of the way. This
        ...
        let ebpf_target_dir = out_dir.join("secure-x-ebpf");
